...
public class SettlementLogImpl implements SettlementLog {
    ...
    protected final @NotNull Map<ProductLocation, PriceInstance> pricePerStack = new HashMap<>();
    ...
    @Override
    public @NotNull SettlementLog setPricePerStack(@NotNull Map<ProductLocation, PriceInstance> pricePerStack) {
    ...
    @Override
    public @NotNull @Unmodifiable Map<ProductLocation, PriceInstance> getPricePerStack() {
    ...
    public @NotNull Map<@NotNull String, @NotNull Double> getTotalPrices() {
        ...
        for (Map.Entry<ProductLocation, AmountPair> orderedProduct : orderedProducts.entrySet()) {
            final PriceInstance priceInstance = pricePerStack.get(orderedProduct.getKey());
            final int stack = orderedProduct.getValue().stack();
            if (priceInstance == null) continue;
